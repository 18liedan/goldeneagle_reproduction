...
clutch <- data%>%drop_na(natclutch) 
    #remove rows with unknown natural clutches (due to clutch supplementation)
    #use this dataset for predicting the number of eggs laid
...
clutch_gen <- clutch%>%drop_na(aadisPBR)
hatchable_gen <- hatchable%>%drop_na(aadisPBR)
datafert_gen <- datafert%>%drop_na(aadisPBR)
data_yn_gen <- data_yn%>%drop_na(aadisPBR)


##Predict number of eggs laid with non-genetic factors##
#test all combinations
clutch1 <- glmmTMB(natclutch~studrel+studrel.2+repyears+age_m+age_f+
                     (1|male)+(1|female)+(1|zoo),family=compois, data=clutch)
  summary(clutch1)
  options(na.action = "na.fail")
  clutch1_dredge = dredge(clutch1)
  print(clutch1_dredge)
  write.csv(clutch1_dredge, file = "clutch1_dredge.csv")

#models with dAIC < 2
clutch1.1 <- glmmTMB(natclutch~repyears+studrel+studrel.2+
              (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.1)
  AICc(clutch1.1)
  testDispersion(clutch1.1) #ok
  diagnose(clutch1.1) #large z
  plotQQunif(clutch1.1)
  plotResiduals(clutch1.1)
  
clutch1.2 <- glmmTMB(natclutch~age_m+repyears+(1|male)+(1|female)+(1|zoo),
                          family=compois, data=clutch)
  summary(clutch1.2)
  AICc(clutch1.2)
  testDispersion(clutch1.2) #ok
  diagnose(clutch1.2) #unusually large z stat
  plotQQunif(clutch1.2)
  plotResiduals(clutch1.2)
  
clutch1.3 <- glmmTMB(natclutch~age_f+age_m+(1|male)+(1|female)+(1|zoo),
                          family=compois, data=clutch)
  summary(clutch1.3)
  AICc(clutch1.3)
  testDispersion(clutch1.3) #ok
  diagnose(clutch1.3) #unusually large z stat
  plotQQunif(clutch1.3)#deviating...
  plotResiduals(clutch1.3) #deviating...
  
clutch1.4 <- glmmTMB(natclutch~age_m+repyears+studrel+studrel.2+
                  (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.4)
  AICc(clutch1.4)
  testDispersion(clutch1.4) #ok
  diagnose(clutch1.4) #unusually large z stat
  plotQQunif(clutch1.4)
  plotResiduals(clutch1.4) #ok!
  
clutch1.5 <- glmmTMB(natclutch~age_f+repyears+studrel+studrel.2+
                  (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.5)
  AICc(clutch1.5)
  testDispersion(clutch1.5) #ok
  diagnose(clutch1.5) #unusually large z stat
  plotQQunif(clutch1.5)
  plotResiduals(clutch1.5) #ok!
  
clutch1.6 <- glmmTMB(natclutch~age_f+studrel+studrel.2+
                (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.6)
  AICc(clutch1.6)
  testDispersion(clutch1.6) #ok
  diagnose(clutch1.6) #unusually large z stat
  plotQQunif(clutch1.6)
  plotResiduals(clutch1.6) #ok!
  
clutch1.7 <- glmmTMB(natclutch~age_m+age_f+repyears+
                  (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.7)
  AICc(clutch1.7)
  testDispersion(clutch1.7) #ok
  diagnose(clutch1.7) #unusually large z stat
  plotQQunif(clutch1.7)
  plotResiduals(clutch1.7) #ok!

clutch1.8 <- glmmTMB(natclutch~age_m+studrel+repyears+
                  (1|male)+(1|female)+(1|zoo), family=compois, data=clutch)
  summary(clutch1.8)
  AICc(clutch1.8)
  testDispersion(clutch1.8) #ok
  diagnose(clutch1.8) #unusually large z stat
  plotQQunif(clutch1.8)
  plotResiduals(clutch1.8) #ok!
  
#plots
jpeg("clutch_repyears.jpg",res=600, width=4000, height=3000, pointsize=10)
plot(ggpredict(clutch1.2, terms=c("repyears")))+
    geom_count(data=clutch, stroke = NA, aes(repyears,natclutch), 
    alpha = 0.7, stat = "sum", position = "identity")
dev.off()

jpeg("clutch_agem.jpg",res=600, width=4000, height=3000, pointsize=10)
plot(ggpredict(clutch1.2, terms=c("age_m")))+
    geom_count(data=clutch, stroke = NA, aes(age_m,natclutch), 
    alpha = 0.7, stat = "sum", position = "identity")
dev.off()

jpeg("clutch_studrel.jpg",res=600, width=4000, height=3000, pointsize=10)
plot(ggpredict(clutch1.5, terms=c("studrel")))+
    geom_count(data=clutch, stroke = NA, aes(studrel,natclutch), 
    alpha = 0.7, stat = "sum", position = "identity")
dev.off()

jpeg("clutch_agef.jpg",res=600, width=4000, height=3000, pointsize=10)
plot(ggpredict(clutch1.6, terms=c("age_f")))+
    geom_count(data=clutch, stroke = NA, aes(age_f,natclutch), 
    alpha = 0.7, stat = "sum", position = "identity")
dev.off()


##Predict hatching (yes/no) with non-genetic factors##
#test all combinations
...
##Predict number of eggs laid with genetic factors##
#test all combinations
clutch2 <- glmmTMB(natclutch~aadisPBR+aadisPBR.2+pd+pd.2+nam+naf+
                     (1|male)+(1|female)+(1|zoo), family=compois, data=clutch_gen)
  summary(clutch2)
  options(na.action = "na.fail")
  clutch2_dredge = dredge(clutch2)
  print(clutch2_dredge)
  write.csv(clutch2_dredge, file = "clutch2_dredge.csv")
  
#models with dAIC < 2
clutch2.1 <- glmmTMB(natclutch~naf+aadisPBR.2+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.1)
  AICc(clutch2.1)
  testDispersion(clutch2.1) #ok
  diagnose(clutch2.1) #large z stat
  plotQQunif(clutch2.1) #deviating... 
  plotResiduals(clutch2.1) #deviating...
  
clutch2.2 <- glmmTMB(natclutch~naf+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.2)
  AICc(clutch2.2)
  testDispersion(clutch2.2) #ok
  diagnose(clutch2.2) #large z stat
  plotQQunif(clutch2.2) #deviating... 
  plotResiduals(clutch2.2) #ok  

clutch2.3 <- glmmTMB(natclutch~aadisPBR+naf+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.3) #nonconvergent
  AICc(clutch2.3)
  testDispersion(clutch2.3) #ok
  diagnose(clutch2.3) #large zi stat
  plotQQunif(clutch2.3) #deviating... 
  plotResiduals(clutch2.3) #deviating... 
  
clutch2.4 <- glmmTMB(natclutch~aadisPBR.2+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.4)
  AICc(clutch2.4)
  testDispersion(clutch2.4) #ok
  diagnose(clutch2.4) #large z stat
  plotQQunif(clutch2.4) #ok 
  plotResiduals(clutch2.4) #deviating... 
  
clutch2.5 <- glmmTMB(natclutch~aadisPBR+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.5) 
  AICc(clutch2.5)
  testDispersion(clutch2.5) #ok
  diagnose(clutch2.5) #large z stat
  plotQQunif(clutch2.5) #deviating... 
  plotResiduals(clutch2.5) #ok 
  
clutch2.6 <- glmmTMB(natclutch~pd.2+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.6) 
  AICc(clutch2.6)
  testDispersion(clutch2.6) #ok
  diagnose(clutch2.6) #large z stat
  plotQQunif(clutch2.6) #deviating... 
  plotResiduals(clutch2.6) #deviating... 
  
clutch2.7 <- glmmTMB(natclutch~pd+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.7)
  AICc(clutch2.7)
  testDispersion(clutch2.7) #ok
  diagnose(clutch2.7) #large z stat
  plotQQunif(clutch2.7) #deviating... 
  plotResiduals(clutch2.7) #deviating... 
  
clutch2.8 <- glmmTMB(natclutch~naf+pd.2+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.8)
  AICc(clutch2.8)
  testDispersion(clutch2.8) #ok
  diagnose(clutch2.8) #large z stat
  plotQQunif(clutch2.8) #ok 
  plotResiduals(clutch2.8) #top res dev  

clutch2.9 <- glmmTMB(natclutch~naf+pd+(1|male)+(1|female)+(1|zoo), 
                          family=compois, data=clutch_gen)
  summary(clutch2.9)
  AICc(clutch2.9)
  testDispersion(clutch2.9) #ok
  diagnose(clutch2.9) #large z stat
  plotQQunif(clutch2.9) #ok 
  plotResiduals(clutch2.9) #bottom res dev  
  
#plots
plot(ggpredict(clutch2.2, terms=c("aadisPBR")))+
       geom_count(data=clutch_gen, stroke = NA, aes(aadisPBR,natclutch), 
                alpha = 0.7, stat = "sum", position = "identity")  
